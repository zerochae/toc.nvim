local Parser = require "toc.parsers.base"

---@class Help : Parser
---@field section integer level of the current === / --- section
---@field expect integer|nil a rule was seen; the next non-blank line is its title
---@field in_code boolean inside a `>` code block
local Help = setmetatable({}, { __index = Parser })
Help.__index = Help

---@param bufnr integer
---@return Help
function Help.new(bufnr)
  local self = Parser.new(bufnr) --[[@as Help]]
  self.section = 0
  self.expect = nil
  self.in_code = false
  return setmetatable(self, Help)
end

-- Lines whose only non-space content is >= 3 of `ch` are Vim help section rules.
---@param line string
---@param ch string
---@return boolean
local function is_rule(line, ch)
  return line:match("^" .. ch .. "+%s*$") ~= nil and #line:gsub("%s", "") >= 3
end

-- Strip inline/trailing *tags* and surrounding whitespace from a title.
---@param s string
---@return string
local function clean(s)
  s = s:gsub("%*[^%s*]+%*", "")
  s = s:gsub("^%s+", "")
  s = s:gsub("%s+$", "")
  return s
end

local CALLOUTS = { note = true, warning = true, tip = true, caution = true, deprecated = true }

---@param i integer
---@param line string
function Help:scan(i, line)
  -- Inside a `>` block: an unindented line (or a `<`) ends it.
  if self.in_code then
    if line:match "^<" then
      self.in_code = false
      return
    elseif line == "" or line:match "^%s" then
      return
    end
    self.in_code = false -- unindented line: block ended, process it below
  end

  if self.expect then
    if line:match "%S" then
      if not is_rule(line, "=") and not is_rule(line, "%-") then
        local text = clean(line)
        if text ~= "" and self:enabled "heading" then
          self.section, self.head_level = self.expect, self.expect
          self:add { lnum = i, level = self.expect, kind = "heading", text = text }
        end
      end
      self.expect = nil
    end
    return
  end

  if is_rule(line, "=") then
    self.expect = 1
    return
  elseif is_rule(line, "%-") then
    self.expect = 2
    return
  end

  local sub = line:match "^(%S.-)%s*~%s*$"
  if sub then
    self.head_level = math.max(self.section, 1) + 1
    if self:enabled "heading" then
      self:add { lnum = i, level = self.head_level, kind = "heading", text = clean(sub) }
    end
    return
  end

  local word, rest = line:match "^%s*([%a]+):%s*(.*)$"
  if word and CALLOUTS[word:lower()] then
    if self:enabled "callout" then
      self:add {
        lnum = i,
        level = self:child_level(),
        kind = "callout",
        text = rest ~= "" and clean(rest) or word,
        label = word:lower(),
      }
    end
    return
  end

  -- A line ending in ">" or ">{lang}" (e.g. ">lua") opens a code block.
  local lang = line:match "%s>([%w_]*)%s*$" or line:match "^>([%w_]*)%s*$"
  if lang then
    if self:enabled "code" then
      self:add { lnum = i, level = self:child_level(), kind = "code", text = lang ~= "" and lang or "code" }
    end
    self.in_code = true
    return
  end

  if self:enabled "link" then
    for tag in line:gmatch "%*([^%s*|]+)%*" do
      self:add { lnum = i, level = self:child_level(), kind = "link", text = tag }
    end
  end
end

---Parse a Vim help buffer into typed entries (unsorted).
---  `===` / `---` rules  -> level 1 / 2 section headings (title on the next line)
---  `Foo ~`              -> subheading, one level under the current section
---  `> ... <`            -> code block
---  `Note:` / `WARNING:` -> callout
---  `*tag*`              -> anchor (link)
---@return TocEntry[]
function Help:parse()
  for i, line in ipairs(self:lines()) do
    self:scan(i, line)
  end
  return self.entries
end

return Help
